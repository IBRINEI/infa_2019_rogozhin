...
class Ball(Agent):
    ...
    def update(self):
        if self.x + self.r > self.xbound:
            self.x -= self.vx
            self.vx = - self.vx * 0.6
        if self.y + self.r > self.ybound:
            self.y -= self.vy
            self.vy = - self.vy * 0.8
            self.vx *= 0.8
        else:
...
class Target(Agent):
    def __init__(
            self, canvas, x=None, y=None, r=None, color=None, job_init=None):  # TODO
        super().__init__()
        self.job = job_init
    ...
    def destroy(self):
        ...
        for key, item in self.canvas.targets.items():
            if item is self:
                del self.canvas.targets[key]
                break
...
class BattleField(tk.Canvas):
    ...
    def create_targets(self):
        for i in range(self.num_targets):
            ...
            self.targets.update({i: Target(self)})
